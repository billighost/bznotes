...
def like_story(story_id):
    ...
    if current_user.id != story.user_id:  
        send_notification(story.user_id, f"{current_user.username} liked your story: {story.title}")

    db.session.commit()  # ✅ Now commit changes
    ...
    return jsonify({"liked": liked, "likes": likes_count, "success": True})
...
def determine_likes():
    ...
    form = DetermineLikesForm()
    
    if form.validate_on_submit():
        flash("Likes settings updated!", "success")
        return redirect(url_for("dashboard"))

    return render_template("determine_likes.html", form=form)
